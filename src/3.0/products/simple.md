...
<ImagePopup src="/assets/3.0/images/simple-product/simple.png" alt="Create Simple Product modal" />

UnoPim closes the dialog and redirects you to the edit page, where every remaining field can be filled in.

<ImagePopup src="/assets/3.0/images/simple-product/editProduct.png" alt="Edit Simple Product" />

### The edit page layout

The 3.0 edit page is built to stay fast even for very large attribute families:

- **Attribute group panels** — every attribute group of the family appears as its own collapsible panel down the main column. Click a panel header to open or close it; UnoPim remembers your choice per group.
- **Groups load as you scroll** — only the first groups are loaded when the page opens, so products with hundreds or thousands of attributes open quickly. The remaining groups load automatically as you scroll down. If a group fails to load, a **Retry** button appears — click it to try again.
- **Errors are never hidden** — if you save and a problem is found in a group you have not scrolled to yet, UnoPim loads that group and takes you straight to the field.
- **Product navigation rail** — on the right, a collapsible rail holds the **Categories** and **Associations** cards plus a **Product Info** summary (status, family, type, created and updated dates). Each card shows a quick summary — for example how many products are linked. Clicking a card slides open a drawer over the page where you make your changes; see [Linking Products](../associations/linking-products.md) for the Associations drawer.
- **One Save / Discard bar** — there is no separate save button per section. As soon as you change anything, the global bar appears; **Save** stores everything at once and **Discard** puts it back. See [Admin Essentials](../introduction/admin-essentials.md).
...
Each section appears as its own card on the edit page. The **Short Description** and **Description** cards include a WYSIWYG editor for rich text. The **Price** card shows one row per configured currency. The **Technical** card holds the green **Status** toggle — leave it *Enabled* for the product to be considered active. The **Categories** card in the right rail opens a drawer with a searchable category tree; tick every node the product belongs to and click **Add Selected**. All sections are saved together by the global **Save / Discard** bar.

### Step 3 — Add associations

The **Associations** card in the right rail opens the Associations drawer, where you link this product to others. Each association type is a tab; in every tab you click **Add**, pick products from the product grid, and confirm with **Add Selected**. See [Linking Products](../associations/linking-products.md) for the full walkthrough.
...
Linked products appear as a list under each tab with a **Remove Product** action, and there is no limit on how many you can add.

### Step 4 — Save

Click **Save** on the **Save / Discard** bar that appeared when you started editing. You stay on the edit page, so you can keep working. Back in the **Products Data Grid**, the new product appears as a row with its SKU, image thumbnail, name, attribute family, status chip, type (*Simple*), and completeness percentage.

<ImagePopup src="/assets/3.0/images/simple-product/datagrid.png" alt="Products Datagrid" />
...
<ImagePopup src="/assets/3.0/images/simple-product/product-edit-locale.png" alt="Product Edit with Locale Switcher" />
...
5. Click **Save** on the Save / Discard bar.
6. Repeat per locale.
...
<ImagePopup src="/assets/3.0/images/simple-product/columns-selector.png" alt="Columns Selector" />
