...
- **Temperature** — how creative or deterministic the output is, on a scale from 0 to 2 (low = tight and repeatable, high = varied and inventive).
- **Max Output Tokens** — how long the response can be (100 to 5000 tokens).
...
<ImagePopup src="/assets/3.0/images/magic-ai/system-prompts.png" alt="System Prompts" />
...
- **Prompt Tone** — A plain-language description of the voice. The model reads this at request time, so be specific: *"Write in an understated, elegant tone. Use concise sentences. Avoid marketing hyperbole."*
- **Max Output Tokens** — Caps response length, from 100 to 5000 tokens. Lower values produce shorter, cheaper output; higher values give the model more room.
- **Temperature** — 0 to 2. Low values (e.g., 0.4) are best for reliable, repeatable output; higher values add variety and flair.
...
| **0.8 and above** | Lifestyle content, storytelling, blog-style copy — where variety and creativity shine. |
