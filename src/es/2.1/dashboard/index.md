...
<ImagePopup src="/assets/2.1/images/dashboard/dashboard-overview.png" alt="Visión general del Panel" />
...
|---|---|---|
| **Total Products** | Conteo de productos en todos los estados y tipos. | Página de listado de productos. |
| **Total Categories** | Conteo de categorías en todo el árbol. | Página de listado de categorías. |
...
::: tip Mosaicos de estadísticas pulsables
Cada mosaico de Estadísticas de producto ahora actúa como un **chip de filtro** — haga clic en *Activos*, *Inactivos*, *Con variantes*, *Enriquecidos* o *Nuevos esta semana* y el Panel le enlazará directamente al listado de Productos prefiltrado a ese conjunto. No es necesario recrear el filtro manualmente.
:::
...
<ImagePopup src="/assets/2.1/images/settings/dark-theme.png" alt="Tema Oscuro" />
...
## Refrescar la caché del Panel

Los widgets más pesados del Panel — recuentos totales, estadísticas de producto, preparación de canal y la lista Requiere atención — están en caché para que la página cargue instantáneamente incluso en catálogos grandes. Las entradas de caché son invalidadas automáticamente por las escrituras (crear un producto borra `dashboard.product_stats`, etc.), por lo que en uso normal siempre debería ver datos frescos.

Si alguna vez necesita **forzar una actualización** — por ejemplo después de una importación masiva en base de datos que escribió fuera de la capa de repositorio normal, o para confirmar que una estadística está genuinamente obsoleta en lugar de simplemente cacheada — ejecute:

```sh
php artisan unopim:dashboard:refresh
```

El comando borra estas cinco claves de caché:
...
La siguiente solicitud que llegue al Panel las recomputará desde la base de datos.

::: tip Cuándo ejecutarlo
Casi nunca durante la operación normal — UnoPim invalida la caché por usted. Recurra a este comando cuando:

- Ha importado datos directamente con SQL (saltándose el pipeline de importación).
- Ha restaurado una copia de seguridad de base de datos y el Panel muestra cifras anteriores a la restauración.
- Ha construido una integración personalizada que escribe a través de `DB::table()` en lugar de los repositorios de UnoPim.
:::

::: tip Programación
Puede llamar a `unopim:dashboard:refresh` desde su programador (p. ej., una vez por hora) si tiene sistemas externos escribiendo en la base de datos. Añádalo a `app/Console/Kernel.php` junto a las otras programaciones de UnoPim.
:::
