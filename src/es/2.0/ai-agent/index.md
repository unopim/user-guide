...
Cada mensaje de chat fluye a través de este bucle:

1. **Usted envía un mensaje** en el panel del AI Agent Chat.
2. **El agente interpreta su intención** usando la Platform/Model configurada bajo **Magic AI → Ajustes → Agentic PIM**, más la personalidad activa del System Prompt y cualquier hecho que haya recordado sobre usted.
3. **El agente elige herramientas a llamar** de una biblioteca de más de 30 herramientas PIM (consulte la página [AI Agent Chat](./ai-agent-chat.md) para la lista completa). Para solicitudes complejas encadena varias herramientas juntas en un plan.
4. **Cada herramienta se ejecuta contra datos reales de UnoPim** — pero **solo dentro de sus permisos ACL**. Una herramienta para la que no tiene permiso silenciosamente se rehúsa a ejecutar.
5. **Los cambios destructivos o de baja confianza van a la Approval Queue** (si su Change Approval Mode está configurado así) para que pueda revisarlos antes de aplicarlos.
6. **La respuesta se transmite de vuelta al chat** en tiempo real sobre Server-Sent Events (SSE), por lo que ve el progreso a medida que el agente piensa y actúa.
...
### Gestión de Asociaciones *(Nuevo en v2.0.x)*
Añada, elimine, liste o refleje productos relacionados, up-sells y cross-sells a través de la conversación — sin necesidad de abrir cada página de edición de producto.
