...
Chaque message de chat passe par cette boucle :

1. **Vous envoyez un message** dans le panneau AI Agent Chat.
2. **L'agent interprète votre intention** à l'aide de la Platform/du Model configuré sous **Magic AI → Settings → Agentic PIM**, ainsi que la personnalité System Prompt active et les faits qu'il a mémorisés à votre sujet.
3. **L'agent choisit les outils à appeler** dans une bibliothèque de plus de 30 outils PIM (voir la page [AI Agent Chat](./ai-agent-chat.md) pour la liste complète). Pour les requêtes complexes, il enchaîne plusieurs outils ensemble dans un plan.
4. **Chaque outil s'exécute sur les vraies données UnoPim** — mais **uniquement dans le cadre de vos autorisations ACL**. Un outil pour lequel vous n'avez pas d'autorisation refuse silencieusement de s'exécuter.
5. **Les changements destructifs ou à faible confiance vont dans la file d'attente d'approbation** (si votre Change Approval Mode est défini ainsi) afin que vous puissiez les examiner avant qu'ils ne soient appliqués.
6. **La réponse est diffusée en streaming dans le chat** en temps réel via Server-Sent Events (SSE), afin que vous voyiez la progression à mesure que l'agent réfléchit et agit.
...
### Gestion des associations *(Nouveau en v2.0.x)*
Ajoutez, supprimez, listez ou reproduisez les produits associés, les up-sells et les cross-sells via la conversation — pas besoin d'ouvrir chaque page d'édition de produit.
