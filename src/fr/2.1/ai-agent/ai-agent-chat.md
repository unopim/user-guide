...
- **Sessions** — Vos conversations passées. Un badge numérique sur l'onglet indique combien de sessions sont non lues.
 ...
 <ImagePopup src="/assets/2.1/images/ai-agent/ai-agent-chat-tab.png" alt="Onglet AI Agent Chat" />
...
| 11 | **Bulk Import CSV** | Téléverser un CSV/XLSX pour mise à jour par lots — s'exécute en tant que job mis en file d'attente en arrière-plan  |
...
| 24 | **Manage Associations** | Ajouter, supprimer ou lister les produits liés/up-sell/cross-sell via le langage naturel  |
...
## Bulk Import CSV — désormais en file d'attente

L'outil **Bulk Import CSV** traitait auparavant l'intégralité du fichier en ligne, à l'intérieur de la requête HTTP. Cela fonctionnait pour quelques centaines de lignes, mais expirait sur les téléversements plus volumineux. Dans la v2.1.0, l'outil extrait le gros du travail dans un job dédié mis en file d'attente — **`ImportProductsJob`** — afin que vous puissiez pousser de manière fiable **10 000+ produits** dans un seul message d'AI Agent.

### Comment ça fonctionne

1. Vous demandez à l'agent quelque chose comme *"import these products"* et joignez un CSV/XLSX.
2. L'agent valide le fichier, ouvre un enregistrement **Job Track** (visible depuis la page **Suivi de tâches**) et distribue un `ImportProductsJob` à la file d'attente.
3. Le job traite chaque ligne, en enregistrant les résultats dans un `JobTrackBatch`. La requête HTTP originale renvoie immédiatement un ID de job — vous pouvez continuer à chatter pendant que l'import s'exécute.
4. L'agent rapporte la progression dans le chat à mesure que le job avance. Lorsqu'il se termine, vous obtenez un résumé (`created`, `updated`, erreurs).

### Modes d'import

L'agent peut exécuter l'import dans l'un des trois modes — il choisit selon votre formulation, mais vous pouvez être explicite :

| Mode | Comportement | Quand l'utiliser |
|---|---|---|
| **`create_or_update`** *(par défaut)* | Met à jour les SKU existants, crée les nouveaux. | Imports de lots mixtes. |
| **`create_only`** | Crée uniquement de nouveaux SKU — les lignes existantes sont **ignorées**. | Chargements initiaux où vous souhaitez éviter de toucher aux données existantes. |
| **`update_only`** | Met à jour uniquement les SKU existants — les nouveaux SKU sont **ignorés**. | Corrections en masse sur un ensemble de SKU connu. |

### Comportement du job

| Paramètre | Valeur | Notes |
|---|---|---|
| **Timeout** | `3600` secondes (1 heure) | Plafond généreux pour les très gros imports. |
| **Tries** | `1` | Le job n'est **pas** réessayé en cas d'échec — les résultats partiels sont enregistrés et le job est marqué `failed`. Relancez l'import depuis l'AI Agent après avoir corrigé le fichier source. |
| **Queue** | `default` | Assurez-vous que votre liste `queue:work --queue=` inclut `default` (c'est le cas dans la [commande worker documentée](../configuration/webhooks#running-the-queue-worker)). |

### Ce qui est enregistré

Chaque import est distribué dans le **pipeline Transfert de données** standard d'UnoPim, vous pouvez donc le surveiller de la même manière qu'un import CSV normal :

- **Suivi de tâches** → voir cet import dans la liste avec l'état `processing` → `completed`.
- **Résumé par batch** → `processed_rows`, `created`, `updated`, erreurs.
- **Erreurs** → les 10 premières erreurs au niveau ligne sont enregistrées au format `Row N (SKU: X): <message>` afin que vous puissiez corriger le fichier source.

::: tip Pourquoi `$tries = 1` ?
Les imports en masse ne sont pas réessayables en toute sécurité dès le départ — relancer un import à moitié terminé re-traiterait les lignes qui ont déjà réussi, et doublerait le traitement de tout ce qui a échoué en milieu de ligne. UnoPim s'arrête délibérément après une tentative pour vous permettre de décider comment récupérer. Utilisez la liste d'erreurs dans le Suivi de tâches pour corriger les lignes source et réimporter.
:::
...
L'onglet Sessions liste chaque chat passé. Chaque entrée affiche le titre de la session, le nombre de messages et la date de dernière activité.
 ...
 <ImagePopup src="/assets/2.1/images/ai-agent/ai-agent-sessions.png" alt="Sessions AI Agent" />
