...
| **Identifier role** | Marks a field as the **GTIN**, **Model** or **Batch** identifier. These feed the identification block and the QR code rather than appearing as an ordinary row. Any attribute can play the role — if your GTIN lives in `ean`, point the role at `ean`. |
