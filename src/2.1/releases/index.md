...
const features = [
  // v2.1.0
  { title: 'Manage Associations Tool', desc: 'New AI Agent tool that adds, removes, or lists related/up-sell/cross-sell products via natural language — no need to open each product manually. Product search results now display clickable links for streamlined navigation.', version: 'v2.1.0', domains: ['ai', 'productivity'] },
  { title: 'Demo Data Seeding', desc: 'Quick onboarding with sample products, categories, and attributes — available via the installer wizard toggle, the `php artisan unopim:install --with-demo-data` CLI flag, the Docker setup option, or the standalone `php artisan unopim:install:demo-data` command.', version: 'v2.1.0', domains: ['productivity', 'governance'] },
  { title: 'Production-Ready Docker Setup', desc: 'Multi-container stack (Nginx + PHP-FPM default, Apache fallback) shipped via Docker Hub images, with healthchecks, Redis, Elasticsearch, Mailpit services, OPcache-tuned php.ini, and auto-publish workflow.', version: 'v2.1.0', domains: ['connectivity', 'scalability'] },
  { title: 'MagicAI Custom Provider', desc: 'Connect any OpenAI-compatible service (self-hosted gateways, proxies, custom endpoints) by configuring a custom base URL on the Magic AI Platforms page — beyond the official OpenAI/Anthropic/Gemini/Ollama/Groq presets.', version: 'v2.1.0', domains: ['ai', 'connectivity'] },
  { title: 'MagicAI Test Connection (ModelRecommender)', desc: 'Test Connection on a Platform now uses a dedicated recommender that skips image-only models, preventing false negatives during credential validation.', version: 'v2.1.0', domains: ['ai', 'governance'] },
  { title: 'PrismErrorResolver', desc: 'AI Agent Chat translates provider and Prism errors into user-friendly messages, so admins immediately understand what went wrong (rate limit, invalid key, model unavailable, etc.).', version: 'v2.1.0', domains: ['ai', 'productivity'] },
  { title: 'Clickable Dashboard Product Stats', desc: 'Product-stats cards on the Dashboard (Active, Inactive, Enriched, With Variants, …) now act as filter chips — clicking one deep-links into the product grid pre-filtered to that set.', version: 'v2.1.0', domains: ['productivity', 'reporting'] },
  { title: 'Async Product Webhook Dispatch', desc: 'Product create/update webhook deliveries now run as a queued SendProductWebhook job, so admin actions return immediately and external endpoint latency no longer blocks the admin UI.', version: 'v2.1.0', domains: ['connectivity', 'automation', 'scalability'] },
  { title: 'Admin Login Rate Limiting', desc: 'Named rate limiters with per-email + per-IP segmentation (5 attempts/minute) protect the admin login and forgot-password endpoints from brute-force attacks.', version: 'v2.1.0', domains: ['governance', 'scalability'] },
  { title: 'Server-Side Password Validation', desc: 'Minimum 6-character password length enforced server-side in UserForm, closing a gap where clients could bypass password rules.', version: 'v2.1.0', domains: ['governance'] },
  { title: 'User-Enumeration Hardening', desc: 'Forgot-password endpoint now returns a generic message regardless of whether the email exists, preventing account enumeration via response differences.', version: 'v2.1.0', domains: ['governance'] },
  { title: 'Open-Redirect Protection', desc: 'Referer-based redirects on login and forgot-password now validate the host via parse_url() so spoofed Referer headers can no longer push users to external phishing sites.', version: 'v2.1.0', domains: ['governance'] },
  { title: 'User-Edit Privilege Escalation Fix', desc: 'Added missing ACL entries and controller-level guards on the user update and destroy endpoints; non-superadmins can no longer assign permission_type: all roles. Patches the High-severity audit finding.', version: 'v2.1.0', domains: ['governance'] },
  { title: 'No-Cache Middleware for Admin Pages', desc: 'Admin responses include Cache-Control: no-store and Pragma: no-cache headers that prevent browsers and proxies from caching sensitive pages after sign-out.', version: 'v2.1.0', domains: ['governance'] },
  { title: 'Enhanced Secure Headers', desc: 'Permissions-Policy and X-Permitted-Cross-Domain-Policies headers are now sent on all admin responses for stronger browser-level protections.', version: 'v2.1.0', domains: ['governance'] },
  { title: 'XSS Sanitization Helper', desc: 'clean_content() helper backed by HTMLPurifier strips Blade directives, PHP tags, and dangerous HTML from user-generated content before storage.', version: 'v2.1.0', domains: ['governance'] },
  { title: 'IP-Based Debug Filtering', desc: 'APP_DEBUG_ALLOWED_IPS env variable restricts debugbar access to specific IPs so production debugging is safer.', version: 'v2.1.0', domains: ['governance'] },
  { title: 'Improved Dutch (nl_NL) Translations', desc: 'Quality pass across nine packages (Admin, AiAgent, Completeness, Core, DataTransfer, Installer, Product, Webhook) — informal tone, stray prefix removal, zero-width-space cleanup. Community contribution.', version: 'v2.1.0', domains: ['productivity'] },
  { title: 'Database Index Performance Pass', desc: 'New indexes on channels.code, locales.status, currencies.status, and a composite index on core_config(code, channel_code, locale_code) for faster config lookups and admin page loads.', version: 'v2.1.0', domains: ['scalability'] },
  { title: 'PostgreSQL Compatibility Pass', desc: 'JSON search support, jsonContains() grammar, driver-gated MySQL bulk-mode session vars, and DataGrid filter fixes across AiAgent tools, Channel, and AttributeOption grids.', version: 'v2.1.0', domains: ['connectivity', 'scalability'] },
  { title: 'Dark-Mode Polish in Job Tracker', desc: 'Progress bars and configuration buttons now render correctly in dark mode inside the import/export job tracker.', version: 'v2.1.0', domains: ['productivity'] },
  { title: 'PostgreSQL CI Workflow', desc: 'Dedicated Pest test run against PostgreSQL 16 (with and without Elasticsearch) on every change keeps cross-database compatibility stable.', version: 'v2.1.0', domains: ['scalability', 'governance'] },
