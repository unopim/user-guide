...
Der Capabilities-Tab listet jedes Tool auf, das der Agent aufrufen kann. Jedes Tool repräsentiert eine bestimmte PIM-Operation, die Sie über natürliche Sprache auslösen können — Sie rufen Tools nicht namentlich auf, sondern Sie beschreiben, was Sie wollen, und der Agent wählt das richtige aus.
...
| 11 | **Bulk Import CSV** | CSV/XLSX hochladen für Batch-Update — läuft als in die Warteschlange eingereihter Hintergrundjob  |
...
| 24 | **Manage Associations** | Verwandte/Up-Sell-/Cross-Sell-Produkte über natürliche Sprache hinzufügen, entfernen oder auflisten  |
...
## Bulk Import CSV — jetzt in der Warteschlange

Das **Bulk Import CSV**-Tool verarbeitete zuvor die gesamte Datei inline innerhalb der HTTP-Anfrage. Das funktionierte für ein paar hundert Zeilen, lief jedoch bei größeren Uploads in einen Timeout. In v2.1.0 extrahiert das Tool die Schwerstarbeit in einen dedizierten in die Warteschlange eingereihten Job — **`ImportProductsJob`** — sodass Sie zuverlässig **mehr als 10.000 Produkte** in einer einzigen AI-Agent-Nachricht pushen können.

### So funktioniert es

1. Sie bitten den Agenten z. B. *"importiere diese Produkte"* und hängen eine CSV/XLSX an.
2. Der Agent validiert die Datei, öffnet einen **Job Track**-Datensatz (sichtbar auf der **Job-Tracker**-Seite) und versendet einen `ImportProductsJob` an die Warteschlange.
3. Der Job verarbeitet jede Zeile und zeichnet die Ergebnisse in einem `JobTrackBatch` auf. Die ursprüngliche HTTP-Anfrage antwortet sofort mit einer Job-ID — Sie können weiter chatten, während der Import läuft.
4. Der Agent meldet den Fortschritt im Chat, während der Job voranschreitet. Wenn er abgeschlossen ist, erhalten Sie eine Zusammenfassung (`created`, `updated`, Fehler).

### Import-Modi

Der Agent kann den Import in einem von drei Modi ausführen — er wählt basierend auf Ihrer Formulierung, aber Sie können explizit sein:

| Modus | Verhalten | Wann zu verwenden |
|---|---|---|
| **`create_or_update`** *(Standard)* | Aktualisiert vorhandene SKUs, erstellt neue. | Gemischte Batch-Importe. |
| **`create_only`** | Erstellt nur neue SKUs — vorhandene Zeilen werden **übersprungen**. | Erstladungen, bei denen Sie vorhandene Daten nicht berühren wollen. |
| **`update_only`** | Aktualisiert nur vorhandene SKUs — neue SKUs werden **übersprungen**. | Massenkorrekturen auf einer bekannten SKU-Menge. |

### Job-Verhalten

| Einstellung | Wert | Hinweise |
|---|---|---|
| **Timeout** | `3600` Sekunden (1 Stunde) | Großzügige Obergrenze für sehr große Importe. |
| **Tries** | `1` | Der Job wird bei einem Fehler **nicht** wiederholt — Teilergebnisse werden aufgezeichnet und der Job als `failed` markiert. Führen Sie den Import vom AI Agent erneut aus, nachdem Sie die Quelldatei korrigiert haben. |
| **Queue** | `default` | Stellen Sie sicher, dass Ihre `queue:work --queue=`-Liste `default` enthält (das ist in der dokumentierten [Worker-Befehl](../configuration/webhooks#running-the-queue-worker) der Fall). |

### Was wird aufgezeichnet

Jeder Import wird in die standardmäßige UnoPim-**Data-Transfer-Pipeline** verschickt, sodass Sie ihn auf dieselbe Weise überwachen können wie einen normalen CSV-Import:

- **Job-Tracker** → sehen Sie diesen Import in der Liste mit Status `processing` → `completed`.
- **Zusammenfassung pro Batch** → `processed_rows`, `created`, `updated`, Fehler.
- **Fehler** → die ersten 10 Zeilenfehler werden im Format `Row N (SKU: X): <Nachricht>` gespeichert, damit Sie die Quelldatei korrigieren können.

::: tip Warum `$tries = 1`?
Massenimporte sind von vornherein nicht sicher wiederholbar — das erneute Ausführen eines halbfertigen Imports würde Zeilen erneut verarbeiten, die bereits erfolgreich waren, plus alles, was mitten in der Zeile fehlschlug, doppelt verarbeiten. UnoPim bricht bewusst nach einem Versuch ab, damit Sie entscheiden können, wie Sie wiederherstellen. Verwenden Sie die Fehlerliste im Job-Tracker, um die Quellzeilen zu korrigieren und neu zu importieren.
:::
