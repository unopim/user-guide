...
<ImagePopup src="/assets/2.1/images/dashboard/dashboard-overview.png" alt="Dashboard-overzicht" />
...
::: tip Klikbare stattegels
Elke productstatistiek-tegel fungeert nu als een **filterchip** — klik op *Actief*, *Inactief*, *Met Varianten*, *Verrijkt* of *Nieuw deze week* en het Dashboard linkt u direct naar de Productlijst, vooraf gefilterd op die set. Geen noodzaak om het filter handmatig opnieuw te maken.
:::
...
Een zwevende knop **"Open Agenting PIM"** bevindt zich in de rechteronderhoek van het Dashboard (en elke andere admin-pagina). Door erop te klikken opent u de conversationele AI Agent — typ wat u nodig hebt in begrijpelijke taal en hij roept namens u de juiste PIM-tool aan.
...
<ImagePopup src="/assets/2.1/images/settings/dark-theme.png" alt="Donker Thema" />
...
## De Dashboard-cache vernieuwen

De zwaardere widgets van het Dashboard — totale tellingen, productstatistieken, kanaalgereedheid en de 'Vereist aandacht'-lijst — worden gecached zodat de pagina onmiddellijk laadt, zelfs op grote catalogi. Cache-vermeldingen worden automatisch ongeldig gemaakt door schrijfbewerkingen (het aanmaken van een product wist `dashboard.product_stats`, enz.), dus bij normaal gebruik zou u altijd verse gegevens moeten zien.

Als u ooit een **geforceerde vernieuwing** nodig heeft — bijvoorbeeld na een bulkdatabase-import die buiten de normale repository-laag heeft geschreven, of om te bevestigen dat een statistiek werkelijk verouderd is in plaats van slechts gecached — voer dan uit:

```sh
php artisan unopim:dashboard:refresh
```

Het commando wist deze vijf cachesleutels:
...
Het volgende verzoek dat het Dashboard bereikt, zal ze opnieuw berekenen vanuit de database.

::: tip Wanneer uitvoeren
Bijna nooit tijdens normale werking — UnoPim invalideert de cache voor u. Grijp naar dit commando wanneer u:

- Gegevens direct heeft geïmporteerd met SQL (de import-pipeline omzeilend).
- Een database-backup heeft hersteld en het Dashboard pre-restore-cijfers toont.
- Een aangepaste integratie heeft gebouwd die schrijft via `DB::table()` in plaats van UnoPim-repositories.
:::

::: tip Planning
U kunt `unopim:dashboard:refresh` aanroepen vanuit uw scheduler (bijv. een keer per uur) als u externe systemen hebt die naar de database schrijven. Voeg het toe aan `app/Console/Kernel.php` naast de andere UnoPim-schedules.
:::
