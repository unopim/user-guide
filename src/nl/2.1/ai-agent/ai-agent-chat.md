...
Het Platform en Model die voor deze redeneerlus worden gebruikt, worden geconfigureerd onder **Magic AI → Instellingen → Agentic PIM**. De persoonlijkheid (toon, temperatuur, max. tokens) komt uit de actieve **Systeemprompt**.
...
- **Sessions** — Uw eerdere conversaties. Een numerieke badge op het tabblad toont hoeveel sessies ongelezen zijn.
 ...
 <ImagePopup src="/assets/2.1/images/ai-agent/ai-agent-chat-tab.png" alt="AI Agent Chat tabblad" />
...
| 11 | **Bulk Import CSV** | Upload CSV/XLSX voor batch-update — draait als een achtergrondwachtrij-job  |
...
| 24 | **Manage Associations** | Voeg gerelateerde/up-sell/cross-sell-producten toe, verwijder of toon ze via natuurlijke taal  |
...
## Bulk Import CSV — nu in de wachtrij

De tool **Bulk Import CSV** verwerkte voorheen het hele bestand inline, binnen het HTTP-verzoek. Dat werkte voor een paar honderd rijen, maar verliep bij grotere uploads. In v2.1.0 wordt het zware werk uitbesteed aan een speciale wachtrij-job — **`ImportProductsJob`** — zodat u betrouwbaar **10.000+ producten** kunt pushen in één AI Agent-bericht.

### Hoe het werkt

1. U vraagt de agent iets als *"importeer deze producten"* en voegt een CSV/XLSX toe.
2. De agent valideert het bestand, opent een **Job Track**-record (zichtbaar vanaf de **Taakvolger**-pagina) en stuurt één `ImportProductsJob` naar de wachtrij.
3. De job verwerkt elke rij en registreert resultaten in een `JobTrackBatch`. Het oorspronkelijke HTTP-verzoek keert onmiddellijk terug met een job-ID — u kunt blijven chatten terwijl de import loopt.
4. De agent rapporteert voortgang in de chat naarmate de job vordert. Wanneer voltooid, krijgt u een samenvatting (`created`, `updated`, fouten).

### Importmodi

De agent kan de import in een van de drie modi uitvoeren — hij kiest op basis van uw bewoording, maar u kunt expliciet zijn:

| Modus | Gedrag | Wanneer te gebruiken |
|---|---|---|
| **`create_or_update`** *(standaard)* | Werkt bestaande SKU's bij, maakt nieuwe aan. | Gemengde batch-imports. |
| **`create_only`** | Maakt alleen nieuwe SKU's aan — bestaande rijen worden **overgeslagen**. | Eerste-keer-loads waarbij u bestaande gegevens niet wilt aanraken. |
| **`update_only`** | Werkt alleen bestaande SKU's bij — nieuwe SKU's worden **overgeslagen**. | Bulkcorrecties op een bekende SKU-set. |

### Job-gedrag

| Instelling | Waarde | Opmerkingen |
|---|---|---|
| **Timeout** | `3600` seconden (1 uur) | Royaal plafond voor zeer grote imports. |
| **Tries** | `1` | De job wordt **niet** opnieuw geprobeerd bij falen — gedeeltelijke resultaten worden vastgelegd en de job wordt gemarkeerd als `failed`. Voer de import opnieuw uit vanaf de AI Agent nadat u het bronbestand hebt hersteld. |
| **Queue** | `default` | Zorg ervoor dat uw `queue:work --queue=`-lijst `default` bevat (dat is het geval in het gedocumenteerde [worker-commando](../configuration/webhooks#running-the-queue-worker)). |

### Wat wordt vastgelegd

Elke import wordt verzonden in de standaard UnoPim **Gegevensoverdracht-pipeline**, zodat u deze op dezelfde manier kunt monitoren als een normale CSV-import:

- **Taakvolger** → zie deze import in de lijst met status `processing` → `completed`.
- **Per-batch-samenvatting** → `processed_rows`, `created`, `updated`, fouten.
- **Fouten** → de eerste 10 rij-fouten worden opgeslagen met de notatie `Row N (SKU: X): <bericht>` zodat u het bronbestand kunt corrigeren.

::: tip Waarom `$tries = 1`?
Bulkimports zijn vanaf het begin niet veilig opnieuw uit te voeren — een halverwege gestopte import opnieuw uitvoeren zou rijen die al succesvol waren opnieuw verwerken, plus iets dubbel verwerken dat halverwege een rij faalde. UnoPim stopt met opzet na één poging zodat u kunt beslissen hoe te herstellen. Gebruik de foutenlijst in de Taakvolger om de bronrijen te herstellen en opnieuw te importeren.
:::
...
- **RememberFact** — Slaat een feit of voorkeur op waarvan u aangeeft dat de agent het moet onthouden.
...
::: tip
Start een nieuwe sessie wanneer u overschakelt naar een andere taak. Gefocuste sessies leveren betere tool-keuzes op omdat de agent geen onnodige context hoeft af te wegen.
:::
