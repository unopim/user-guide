...
| **Default Platform** | Która platforma obsługuje żądania tekstowe. Wybierz **`-- Use Default Platform --`**, aby podążać za domyślną oznaczoną gwiazdką, lub nadpisz konkretną platformą. Platformy oznaczone `*` w liście rozwijanej są obecnie ustawione jako domyślne. |
