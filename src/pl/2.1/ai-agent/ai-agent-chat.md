...
- **Sessions** — Twoje poprzednie konwersacje. Numeryczna odznaka na zakładce pokazuje, ile sesji jest nieprzeczytanych.
 ...
 <ImagePopup src="/assets/2.1/images/ai-agent/ai-agent-chat-tab.png" alt="Zakładka AI Agent Chat" />
...
Zakładka Capabilities listuje każde narzędzie, które agent może wywołać. Każde narzędzie reprezentuje konkretną operację PIM, którą możesz uruchomić w języku naturalnym — nie wywołujesz narzędzi po nazwie, opisujesz, czego chcesz, a agent wybiera odpowiednie.
...
| 11 | **Bulk Import CSV** | Prześlij CSV/XLSX do masowej aktualizacji — uruchamia się jako zadanie kolejkowane w tle |
...
| 24 | **Manage Associations** | Dodawaj, usuwaj lub listuj produkty powiązane/up-sell/cross-sell w języku naturalnym |
...
## Bulk Import CSV — teraz kolejkowany

Narzędzie **Bulk Import CSV** wcześniej przetwarzało cały plik inline, wewnątrz żądania HTTP. To działało dla kilkuset wierszy, ale wygasało przy większych przesłaniach. W v2.1.0 narzędzie wydziela ciężką pracę do dedykowanego zadania kolejkowanego — **`ImportProductsJob`** — dzięki czemu możesz niezawodnie przesłać **ponad 10 000 produktów** w jednej wiadomości AI Agent.

### Jak to działa

1. Prosisz agenta o coś w stylu *"zaimportuj te produkty"* i dołączasz CSV/XLSX.
2. Agent waliduje plik, otwiera rekord **Job Track** (widoczny ze strony **Śledzenie zadań**) i wysyła jedno `ImportProductsJob` do kolejki.
3. Zadanie przetwarza każdy wiersz, rejestrując wyniki w `JobTrackBatch`. Oryginalne żądanie HTTP wraca natychmiast z ID zadania — możesz nadal czatować, podczas gdy import działa.
4. Agent raportuje postęp w czacie w miarę postępu zadania. Gdy się zakończy, otrzymujesz podsumowanie (`created`, `updated`, błędy).

### Tryby importu

Agent może uruchomić import w jednym z trzech trybów — wybiera na podstawie Twojego sformułowania, ale możesz być wyraźny:

| Tryb | Zachowanie | Kiedy używać |
|---|---|---|
| **`create_or_update`** *(domyślny)* | Aktualizuje istniejące SKU, tworzy nowe. | Importy mieszanych partii. |
| **`create_only`** | Tworzy tylko nowe SKU — istniejące wiersze są **pomijane**. | Pierwsze ładowania, gdy chcesz uniknąć dotykania istniejących danych. |
| **`update_only`** | Aktualizuje tylko istniejące SKU — nowe SKU są **pomijane**. | Masowe poprawki na znanym zestawie SKU. |

### Zachowanie zadania

| Ustawienie | Wartość | Uwagi |
|---|---|---|
| **Timeout** | `3600` sekund (1 godzina) | Hojny pułap dla bardzo dużych importów. |
| **Tries** | `1` | Zadanie **nie** jest ponawiane po niepowodzeniu — częściowe wyniki są rejestrowane, a zadanie jest oznaczane jako `failed`. Uruchom import ponownie z AI Agent po naprawieniu pliku źródłowego. |
| **Queue** | `default` | Upewnij się, że Twoja lista `queue:work --queue=` zawiera `default` (zawiera w udokumentowanym [poleceniu workera](../configuration/webhooks#running-the-queue-worker)). |

### Co jest rejestrowane

Każdy import jest wysyłany do standardowego **pipeline'u Transferu danych** UnoPim, więc możesz go monitorować tak samo, jak monitorujesz normalny import CSV:

- **Śledzenie zadań** → zobacz ten import na liście ze stanem `processing` → `completed`.
- **Podsumowanie per-batch** → `processed_rows`, `created`, `updated`, błędy.
- **Błędy** → pierwsze 10 błędów na poziomie wiersza zapisywanych w formacie `Row N (SKU: X): <message>`, dzięki czemu możesz naprawić plik źródłowy.

::: tip Dlaczego `$tries = 1`?
Importy masowe nie są bezpiecznie ponawialne od początku — ponowne uruchomienie półukończonego importu ponownie przetworzyłoby wiersze, które już się powiodły, oraz podwójnie przetworzyłoby cokolwiek, co nie powiodło się w połowie wiersza. UnoPim celowo wycofuje się po jednej próbie, abyś mógł zdecydować, jak odzyskać dane. Użyj listy błędów w Śledzeniu zadań, aby naprawić wiersze źródłowe i ponownie zaimportować.
:::
...
- **Pole wprowadzania** — Wejście tekstowe na dole, gdzie wpisujesz polecenia lub pytania. Wskazówka klawiaturowa: **Enter**, aby wysłać, **Shift+Enter**, aby utworzyć nową linię.
...
- **Przycisk Wyślij** — Przesyła Twoją wiadomość do AI Agent do przetworzenia.
...
Zakładka Sessions listuje każdy poprzedni czat. Każdy wpis pokazuje tytuł sesji, liczbę wiadomości i datę ostatniej aktywności.
 ...
 <ImagePopup src="/assets/2.1/images/ai-agent/ai-agent-sessions.png" alt="Sesje AI Agent" />
