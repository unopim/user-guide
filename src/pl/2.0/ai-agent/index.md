...
Każda wiadomość czatu przepływa przez tę pętlę:

1. **Wysyłasz wiadomość** w panelu AI Agent Chat.
2. **Agent interpretuje Twoją intencję** używając platformy/modelu skonfigurowanego w **Magic AI → Settings → Agentic PIM** plus aktywnej osobowości System Prompt i wszelkich faktów, które o Tobie zapamiętał.
3. **Agent wybiera narzędzia do wywołania** z biblioteki ponad 30 narzędzi PIM (zobacz stronę [AI Agent Chat](./ai-agent-chat.md), aby zobaczyć pełną listę). Dla złożonych żądań łączy kilka narzędzi w plan.
4. **Każde narzędzie uruchamia się na rzeczywistych danych UnoPim** — ale **tylko w ramach Twoich uprawnień ACL**. Narzędzie, do którego nie masz uprawnień, po cichu odmawia wykonania.
5. **Destrukcyjne lub niskopewne zmiany trafiają do Kolejka zatwierdzeń** (jeśli Twój Change Approval Mode jest tak ustawiony), abyś mógł je przejrzeć, zanim zostaną zastosowane.
6. **Odpowiedź strumieniuje się z powrotem do czatu** w czasie rzeczywistym przez Server-Sent Events (SSE), więc widzisz postęp, gdy agent myśli i działa.

Ponieważ agent ma rzeczywiste narzędzia i rzeczywiste dane, jest potężniejszy niż zwykły czat LLM — ale też bardziej brzemienny w skutkach. Kolejka zatwierdzeń, budżet tokenów, próg pewności i kontrole ACL istnieją, aby utrzymać tę moc pod Twoją kontrolą.
...
### Auto-Enrichment
Powiedz agentowi, aby uzupełnił brakujące opisy, meta-tytuły lub dowolne inne pola tekstowe, a wygeneruje treść pasującą do głosu Twojej marki (poprzez aktywny System Prompt) oraz do szablonów promptów.
...
### Zarządzanie asocjacjami *(Nowość w v2.0.x)*
Dodawaj, usuwaj, listuj lub kopiuj produkty powiązane, up-sells i cross-sells poprzez konwersację — nie trzeba otwierać strony edycji każdego produktu.
